Contributing to NFT Worlds Server
==========================
NFT Worlds is happy you're willing to contribute to our project. We are usually
very lenient with all submitted PRs, but there are still some guidelines you
can follow to make the approval process go more smoothly.

## Use a Personal Fork and not Organization

NFT Worlds will routinely modify your PR, whether it's a quick rebase or to take care
...
Unfortunately, if you use an organization for your PR, it prevents NFT Worlds from
...
- A Java 16 or later JDK (packages vary, use Google/DuckDuckGo/etc.).
  ...
  - NFT Worlds Server requires JDK 16 to build, however makes use of Gradle's
...
The NFT Worlds Server is comprised of patches and extensions to Paper & Spigot servers. These patches/extensions are
split into different directories which target certain parts of the code. These
directories are:

- `NFT-Worlds-API` - Modifications to `Paper-API`/`Spigot-API`/`Bukkit`;
- `NFT-Worlds-Server` - Modifications to `Paper`/`Spigot`/`CraftBukkit`.
...
3. cd into `NFT-Worlds-Server` for server changes, and `NFT-Worlds-API` for API changes.  

## Adding Patches

Adding patches to NFT Worlds is very simple:

1. Modify `NFT-Worlds-Server` and/or `NFT-Worlds-API` with the appropriate changes;
...
These steps assume the `origin` remote is your fork of this repository and `upstream` is the official NFT Worlds Server repository.
...
1. If your PR creates new patches instead of modifying exist ones, in both the `NFT-Worlds-Server` and `NFT-Worlds-API` directories, ensure your newly-created patch is the last commit by either:
...
Remember that these changes will affect everyone who runs the NFT Worlds Server, not just you
and your server.
...
- Multi-line changes start with `// NFT Worlds start` and end with `// NFT Worlds end`;
...
`// NFT Worlds start - reason`.
  ...
  it was before, or what the change is.
   - Multi-line messages should start with `// NFT Worlds start` and use `/* Multi
  line message here */` for the message itself.
- One-line changes should have `// NFT Worlds` or `// NFT Worlds - reason`.
...
When submitting patches to the NFT Worlds Server, we may ask you to add notes to the patch
...
If you add a message to your commit in the `NFT-Worlds-Server`/`NFT-Worlds-API`
...
to make future updates easier on ourselves, NFT Worlds Server tries to use obfuscation
...
`NFTWorldsConfig` class.

### NFTWorldsConfig example
